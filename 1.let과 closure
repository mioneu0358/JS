...
// dovument: HTML에서 생성되는 DOM 객체. 모든 객체들의 root.
...
//         }
//     );
//     //생성된 n은 블록범위익 때문에 for문의 마지막에서 소멸하지만,
...
//outer를 호출하면 내부함수가inner가 반환된다.
//outer의 호출이 끝나면, outer에 대한 컨텍스트는 소멸한다.

var inner= outer();
//outer 컨텍스트는 종료되ㅏ었지만, x의 값은 정상적으로 출력
inner();
